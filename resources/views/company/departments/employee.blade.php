                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div class="hstack gap-2">
                                    <button type="button" id="email-btn" class="btn btn-primary">Email <i class="bx bx-mail-send align-baseline ms-1"></i></button>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="row">
                <div class="col-lg-3">
                    ...
                    <div class="card" id='credit-standing'>
                        <div class="card-body">
                            <h5>{{Auth()->user()->id === $user->id ? 'Your ' : ''}}Credit Standing</h5>
                            <p>{{Auth()->user()->id === $user->id ? 'You have' : $pi->first_name . ' has'}} <span class="fw-bold {{$user->leave_credit === 0 ? 'text-danger' : ($user->leave_credit <= 2 ? 'text-warning' : 'text-primary') }}">{{$user->leave_credit}}</span> leave credit remaining for {{\Carbon\Carbon::now()->year}}</p>
                        </div>
                    </div>
                </div><!--end col-->
                <div class="col-lg-5">
...
<div class="modal fade bs-example-modal-md" id="email-modal" tabindex="-1" role="dialog" aria-labelledby="myExtraLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-md" id="email-modal">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="myExtraLargeModalLabel">Send Email</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <label for="sender" class="form-label mb-2">Send To</label>
                <input type="email" id="recipient" value="{{$ci->email_address}}" name="recipient" class="form-control mb-2">
                <label for="sender" class="form-label mb-2">Sender</label>
                <select name="sender" class="form-control mb-2" id="sender">
                    <option value="{{Auth()->user()->fullname()}}">{{Auth()->user()->fullname()}}</option>
                    <option value="{{Auth()->user()->userCI->email_address}}">{{Auth()->user()->userCI->email_address}}</option>
                    <option value="{{Auth()->user()->company->co_name}}">{{Auth()->user()->company->co_name}}</option>
                    <option value="HRIS">HRIS</option>
                </select>
                <label for="message" class="form-label mb-2">Message</label>
                <textarea name="5" class="form-control" id="message" name="message" placeholder="Write your email" rows="10"></textarea>
            </div>
            <div class="modal-footer">
                <button type="button" id="email-send-btn" class="btn btn-sm btn-primary">Send</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div><!-- /.modal -->
<div class="bg-white py-3 px-4" id="email-spinner" style="display: none; border-radius: 10px; position: absolute; top: 7.5%; right: 2.5%;">
    <div class="d-flex align-items-center gap-3">
        <div class="spinner-border text-primary" role="status">
            <span class="visually-hidden">Loading...</span>
        </div>
        <h4 style="margin-bottom: 0;">Sending Email</h4>
    </div>
</div>
@endsection
...
<script>
    ...
    $('#email-send-btn').click(function() {
        $.ajax({
            url: "{{route('profiling.employees.email')}}",
            data: {
                'sender': $('#sender').val(),
